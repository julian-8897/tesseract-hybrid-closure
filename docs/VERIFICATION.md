...
- All three Tesseract images rebuilt from the working tree, each staging `LICENSE`, `NOTICE` and `ATTRIBUTION.md` under `/tesseract/licenses/` (verified inside the images); container composition smoke passed on the two-step objective, asserting measured solver VJP (≥1) and closure VJP (≥2) calls and a finite non-zero 822,977-parameter gradient.
