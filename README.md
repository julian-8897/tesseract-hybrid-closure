...
<p align="center">
  <img src="docs/figures/hero_hybrid_rollout.gif" width="900" alt="Locked test trajectory comparing a DNS-selected temporal detail from filtered DNS, the hybrid model and the mechanistic solver alone, with full-field context, absolute errors and live rollout MSE">
</p>

The animation shows locked test seed 20000. The top row enlarges the same 26×26 detail in every field; the small insets retain the full 64×64 context. The detail is selected from the DNS alone as the region with greatest mean temporal vorticity change, independently of model errors. Fields share one vorticity scale and error panels share one linear scale. Both display limits use the pooled 99.5th percentile for legibility; clipping is visual only, while the rollout-prefix vorticity MSE uses every cell and time.
